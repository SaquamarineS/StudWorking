...
from app.models import User, Jobs, Review, Application, CompletedOrders
from flask_login import login_user, current_user, logout_user, login_required
import random

rev = [
    {
        'username': 'Жанибек Ерболат',
        'review': 'Думаю о ценности веб сайта'
    },
    {
        'username': 'Рамазан Арман',
        'review': 'Могло быть лучше'
    },
    {
        'username': 'Утегенов Чингис',
        'review': 'Интересный Вебсайт для начинающих'
    }
]
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            if form.usertype.data == user.usertype and form.usertype.data == 'Company':
                ...
                flash('Вход неуспешный. Пожалуйста, проверьте электронную почту, пароль и тип пользователя', 'danger')
        else:
            flash('Вход неуспешный. Пожалуйста, проверьте электронную почту, пароль и тип пользователя', 'danger')
...
def completed_orders():
    completed = CompletedOrders.query.all()
    return render_template('completed_orders.html', completed_orders=completed)
...
def review():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Спасибо за предоставленный обзор!', 'success')
        return redirect(url_for('show_jobs'))
