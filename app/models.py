...
class User(db.Model, UserMixin):
    ...
    def __repr__(self):
        return f"Пользователь('{self.id}', '{self.username}', '{self.usertype}', '{self.email}')"
...
class Application(db.Model):
    ...
    def __repr__(self):
        return f"Заявка('{self.id}','{self.gender}', '{self.date_posted}', '{self.degree}', '{self.industry}', '{self.experience}', '{self.user_id}', '{self.job_id}')"
...
class Jobs(db.Model):
    ...
    def __repr__(self):
        return f"Работы('{self.id}','{self.title}', '{self.industry}', '{self.date_posted}')"
...
class Review(db.Model):
    ...
    def __repr__(self):
        return f"Отзыв('{self.username}', '{self.review}')"
...
class CompletedOrders(db.Model):
    ...
    def __repr__(self):
        return f"ЗавершенныйЗаказ('{self.order_id}', '{self.customer_name}', '{self.date_completed}', '{self.product}')"
